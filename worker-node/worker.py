...
def check_product_flags(product_id):
    ...
    # Robust FSSAI detection:
    # - Handles "fssai" or "lic no" on separate lines
    # - Ignores extra characters, whitespace, colons, dashes
    # - Captures 10-16 digit codes
    fssai_matches = re.findall(
        r"(?:fssai|lic\.?\s*no\.?)\s*[:\-]?\s*\n*\s*(\d{10,16})",
        combined_text,
        re.IGNORECASE
    )
    if fssai_matches:
        flags["HasFSSAI"] = True
        flags["FSSAICodes"] = list(set(fssai_matches))
